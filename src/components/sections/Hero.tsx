  ...
  return (
    <section id="home" className="relative min-h-screen pt-40 lg:pt-48 pb-20 overflow-hidden flex flex-col justify-center bg-background">
      ...
      <div className="container mx-auto px-6 max-w-[1520px] relative z-10 text-center">
        ...
        >
          <span className="inline-block py-1.5 px-4 rounded-full bg-secondary text-primary font-semibold text-sm mb-6 border border-primary/10">
        ...
        >
          ...
          <div className="relative rounded-2xl overflow-hidden shadow-2xl border border-border/50 bg-white/50 backdrop-blur-xl transform rotate-x-12 scale-95 hover:scale-100 hover:rotate-x-0 transition-transform duration-700 ease-out">
            {/* Browser top bar */}
            <div className="h-10 bg-slate-100 border-b border-border flex items-center px-4 gap-2">
              <div className="w-3 h-3 rounded-full bg-red-400" />
              <div className="w-3 h-3 rounded-full bg-amber-400" />
              <div className="w-3 h-3 rounded-full bg-green-400" />
              <div className="mx-auto w-1/2 h-5 bg-white rounded-md shadow-sm border border-slate-200" />
            </div>
            {/* Dashboard content */}
            <div className="h-[400px] md:h-[600px] bg-slate-50 p-6 flex gap-6">
              {/* Sidebar */}
              <div className="w-48 lg:w-64 hidden md:flex flex-col gap-4">
                <div className="h-8 bg-slate-200 rounded w-3/4 mb-4" />
                {[...Array(6)].map((_, i) => (
                  <div key={i} className="h-10 bg-slate-200/50 rounded-lg w-full" />
                ))}
              </div>
              {/* Main Content */}
              <div className="flex-1 flex flex-col gap-6">
                <div className="h-32 bg-white rounded-xl shadow-sm border border-slate-100 p-6 flex justify-between">
                  <div className="h-full w-1/4 bg-blue-50 rounded-lg" />
                  <div className="h-full w-1/4 bg-green-50 rounded-lg" />
                  <div className="h-full w-1/4 bg-purple-50 rounded-lg" />
                </div>
                <div className="flex-1 bg-white rounded-xl shadow-sm border border-slate-100 flex gap-4 p-6">
          ...
          <motion.div
            ...
            className="absolute -right-4 md:-right-12 -bottom-10 md:bottom-20 w-48 md:w-64 h-[400px] md:h-[500px] bg-white rounded-[2.5rem] shadow-2xl border-4 border-slate-800 overflow-hidden flex flex-col"
          >
             <div className="h-full w-full bg-slate-50 relative p-4 flex flex-col gap-4">
                <div className="w-1/2 h-6 bg-slate-200 rounded mx-auto mt-6" />
                <div className="w-full h-32 bg-blue-100 rounded-xl" />
                <div className="flex gap-2">
                  <div className="w-1/2 h-20 bg-white rounded-xl shadow-sm" />
                  <div className="w-1/2 h-20 bg-white rounded-xl shadow-sm" />
                </div>
                <div className="w-full h-12 bg-primary rounded-xl mt-auto shadow-md" />
             </div>
          </motion.div>
        </motion.div>
      </div>
